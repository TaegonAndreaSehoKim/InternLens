...
- Profile Setup uses structured role, skill, location, industry, education, and background inputs with readiness checks.
- Recommendation cards explain matches with fit summaries, matched skills, skill gaps, visible-by-default signals, job detail lookup, and check items.
...
- Local backend tests after account-scoped API updates: **188 passed**
...
- Make ranking components and score explanations easier to inspect.
- Add a repeatable ranking quality report for representative profile types.
- Reduce profile setup friction without making parsed resume data authoritative.
